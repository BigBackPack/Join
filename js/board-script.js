...
function renderLive(tasks) {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    } else if (element[1].board == 'feedback') {
      cardLiveDiv3.innerHTML += renderLiveFeedbackCard(element,i);
    } else if (element[1].board == 'done') {
 ...
 * Renders theOVERLAY for a card based on the provided parameter.
...
function renderDummyOverlayCard(param) {
  ...
  if (param == 'progress') {
    ...
    content.innerHTML = renderDummyOverlayDone();
  } 
}
